	...
	mSearchQueue(searches)
{
	...
	// set up grid
	mTitle = std::make_shared<TextComponent>(mWindow, "RASPADO EN PROGRESO", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
...
void GuiScraperMulti::doNextSearch()
{
	...
	// update subtitle
	ss.str(""); // clear
	ss << "JUEGO " << (mCurrentGame + 1) << " DE " << mTotalGames << " - " << Utils::String::toUpper(Utils::FileSystem::getFileName(mSearchQueue.front().game->getPath()));
...
void GuiScraperMulti::finish()
{
	...
	{
		ss << "NINGÚN JUEGO FUE RASPADO.";
	}else{
		ss << mTotalSuccessful << " JUEGO" << ((mTotalSuccessful > 1) ? "S" : "") << " RASPADO" << ((mTotalSuccessful > 1) ? "S" : "") << " CON ÉXITO!";

		if(mTotalSkipped > 0)
			ss << "\n" << mTotalSkipped << " JUEGO" << ((mTotalSkipped > 1) ? "S" : "") << " SALTADO" << ((mTotalSuccessful > 1) ? "S" : "") << ".";
	}
