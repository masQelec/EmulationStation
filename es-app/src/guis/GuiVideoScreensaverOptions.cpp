...
GuiVideoScreensaverOptions::GuiVideoScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel("CAMBIAR IMAGEN DESPUÉS (SECS)", swap);
	addSaveFunc([swap] {
	...
	addWithLabel("ESTIRAR VIDEO EN EL PROTECTOR DE PANTALLA", stretch_screensaver);
	...
	addWithLabel("USAR REPRODUCTOR OMX PARA EL PROTECTOR DE PANTALLA", ss_omx);
	...
	addWithLabel("MOSTRAR INFORMACIÓN DEL JUEGO EN EL PROTECTOR DE PANTALLA", ss_info);
	...
	auto ss_omx_subs_align = std::make_shared< OptionListComponent<std::string> >(mWindow, "ALINEACIÓN DE INFORMACIÓN DEL JUEGO", false);
	...
	addWithLabel("ALINEACIÓN DE INFORMACIÓN DEL JUEGO", ss_omx_subs_align);
	...
	addWithLabel("TAMAÑO DE FUENTE EN INFORMACIÓN DEL JUEGO", ss_omx_font_size);
	...
	addEditableTextComponent(row, "RUTA DEL ARCHIVO DE FUENTE", ss_omx_font_file, Settings::getInstance()->getString("SubtitleFont"));
	...
	addEditableTextComponent(row, "RUTA DEL ARCHIVO DE FUENTES ITALICAS", ss_omx_italic_font_file, Settings::getInstance()->getString("SubtitleItalicFont"));
	...
	addWithLabel("USAR BAJA RESOLUCIÓN COMPATIBLE PARA LOS SUBTÍTULOS", captions_compatibility);
