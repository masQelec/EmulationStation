	...
	mSystem(system), mMenu(window, "OPCIONES"), fromPlaceholder(false), mFiltersChanged(false)
{
	...
	if (!fromPlaceholder) {
		...
		mJumpToLetterList = std::make_shared<LetterList>(mWindow, "SALTA A...", false);
		...
		row.addElement(std::make_shared<TextComponent>(mWindow, "SALTA A...", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "FILTROS DE LISTA DE JUEGOS", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
	...
	{
		row.elements.clear();
		row.addElement(std::make_shared<TextComponent>(mWindow, "AGREGAR/QUITAR JUEGOS A ESTA COLECCIÓN DE JUEGOS", Font::get(FONT_SIZE_MEDIUM), 0x777777FF), true);
