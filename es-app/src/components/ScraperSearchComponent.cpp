	...
	mSearchType(type)
{
	...
	mResultDesc = std::make_shared<TextComponent>(mWindow, "Resultado desconocido", Font::get(FONT_SIZE_SMALL), 0x777777FF);
...
void ScraperSearchComponent::onSearchDone(const std::vector<ScraperSearchResult>& results)
{
	...
	if(results.empty())
	{
		...
		{
			mWindow->pushGui(new GuiMsgBox(mWindow, Utils::String::toUpper("El raspador configurado ya no está disponible.\nPor favor, cambie la fuente de raspado en la configuración."),
				"FINISH", mSkipCallback));
		}
		else
		{
			ComponentListRow row;
			row.addElement(std::make_shared<TextComponent>(mWindow, "NO SE ENCUENTRAN JUEGOS - SALTAR", font, color), true);
...
void ScraperSearchComponent::openInputScreen(ScraperSearchParams& params)
{
	...
	stop();
	mWindow->pushGui(new GuiTextEditPopup(mWindow, "BÚSQUEDA POR",
