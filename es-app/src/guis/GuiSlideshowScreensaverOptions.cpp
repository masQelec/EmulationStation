...
GuiSlideshowScreensaverOptions::GuiSlideshowScreensaverOptions(Window* window, const char* title) : GuiScreensaverOptions(window, title)
{
	...
	addWithLabel(row, "CAMBIAR IMAGEN DESPUÉS (SECS)", sss_image_sec);
	...
	addWithLabel(row, "ESTIRAR IMÁGENES", sss_stretch);
	...
	addEditableTextComponent(row, "SONIDO DE FONDO", sss_bg_audio_file, Settings::getInstance()->getString("SlideshowScreenSaverBackgroundAudioFile"));
	...
	addWithLabel(row, "UTILICE IMÁGENES PERSONALIZADAS", sss_custom_source);
	...
	addEditableTextComponent(row, "DIRECTORIO DE IMAGEN PERSONALIZADA", sss_image_dir, Settings::getInstance()->getString("SlideshowScreenSaverImageDir"));
	...
	addWithLabel(row, "DIRECTORIO RECURSIVO DE IMAGEN PERSONALIZADA", sss_recurse);
	...
	addEditableTextComponent(row, "FILTRO DE IMAGEN PERSONALIZADO", sss_image_filter, Settings::getInstance()->getString("SlideshowScreenSaverImageFilter"));
