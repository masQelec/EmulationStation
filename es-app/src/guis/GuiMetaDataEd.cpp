	...
	mSavedCallback(saveCallback), mDeleteFunc(deleteFunc)
{
	...
	mTitle = std::make_shared<TextComponent>(mWindow, "EDITAR METADATA", Font::get(FONT_SIZE_LARGE), 0x555555FF, ALIGN_CENTER);
	...
	if(!scraperParams.system->hasPlatformId(PlatformIds::PLATFORM_IGNORE))
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "RASPADO", "scrape", std::bind(&GuiMetaDataEd::fetch, this)));

	buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "SAVAR", "save", [&] { save(); delete this; }));
	...
	if(mDeleteFunc)
	{
		...
		auto deleteBtnFunc = [this, deleteFileAndSelf] { mWindow->pushGui(new GuiMsgBox(mWindow, "ESTO BORRARÁ EL ARCHIVO REAL DEL(OS) JUEGO(S)!\n¿ESTÁS SEGURO?", "SI", deleteFileAndSelf, "NO", nullptr)); };
		buttons.push_back(std::make_shared<ButtonComponent>(mWindow, "ELIMINAR", "delete", deleteBtnFunc));
	}
